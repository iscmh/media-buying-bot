...
export function composeNanoBananaCharacterClonePrompt(
  ...
  return [
    ...
    '{',
    '  "framing": "MEDIUM SHOT — head + shoulders + upper chest visible.',
    "  Subject's face takes up ~30-40% of the frame HEIGHT. Substantial",
    '  headroom (10-15% empty above head) and side room (subject occupies',
    '  center ~50-60% of frame width). NO hands, arms, or phone visible',
    "  in frame — arms are OUT of shot at the subject's sides. This is a",
    '  medium head-and-shoulders shot, NOT a close-up and NOT a full-torso',
    '  selfie. HeyGen Avatar IV zooms in during lip-sync, so leave breathing',
    '  room around the subject or the output will be uncomfortably close",',
    '  "camera": "Feels like a phone-camera POV (mildly wide, ~24mm equiv,',
    '  candid amateur composition) but the phone itself is INVISIBLE — do',
    '  not render a phone, hands, or arms in the frame. The camera appears',
    '  to float at eye-level looking straight at the subject",',
    ...
    '- MEDIUM head-and-shoulders framing — face NO more than ~35% of frame',
    '  height; leave headroom above (10-15% empty) and side room. NOT a',
    '  tight close-up. HeyGen zooms during lip-sync; a tight input becomes',
    '  an uncomfortable output.',
    '- NO phone visible in frame. NO hands or arms visible in frame. The',
    '  shot LOOKS like a phone-selfie POV but the phone/hands are invisible',
    "  (arms are at subject's sides, out of shot). Rendering the phone",
    '  or hands makes HeyGen lip-sync the pose awkwardly.',
    '- No text, no logos, no watermarks, no captions overlaid on the image',
    '- No product hold-ups, no gestures across face',
