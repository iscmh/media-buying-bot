 ...
 */
export const POLISH_VERSION = '25.10.1';
...
export const POLISH_RELEASE_NAME =
  "Polish-25.10 Commit 60 hotfix - Vercel build failure from Commit 59: approve/route.ts exported a `decideVariant` helper alongside the POST handler. Real: Next.js 14 App Router route.ts files reject any export that isn't an HTTP-verb handler (GET/POST/PUT/PATCH/DELETE/HEAD/OPTIONS) or a route-config value (runtime/dynamic/revalidate/preferredRegion/fetchCache/dynamicParams). The build error was: \"Type error: Route ... does not match the required types of a Next.js Route. 'decideVariant' is not a valid Route export field.\" Fix: dropped the trailing `export { decideVariant };` from apps/web/app/api/v1/generations/[id]/variants/[variantId]/approve/route.ts; the helper stays module-local (still used by the POST handler inside the file). Grepped every /api/v1/**/route.ts for non-verb exports - approve was the only offender; reject already inlines its own body so nothing depended on the shared helper. kill + scale + launched_ads + concepts + generations + launches + presets routes are all clean (verify-only). Comment added at the deletion site so a future contributor doesn't re-export by reflex. Version bump: POLISH_VERSION 25.10.0 -> 25.10.1.";
